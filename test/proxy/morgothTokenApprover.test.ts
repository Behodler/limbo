...
*/
  ...
  it("t1. ConfigureTokenApprover approves list of tokens correctly, unapproves list correctly", async function () {
    await expect(SET.configureTokenApproverPower.setApproveOrBlock(SET.goodTokens.map(t => t.address), SET.goodTokens.map(t => true),SET.goodTokens.map(t => true)))
      .to
      .be
    ...
    await SET.configureTokenApproverPower.connect(SET.secondary).setApproveOrBlock(SET.goodTokens.map(t => t.address), SET.goodTokens.map(t => true),SET.goodTokens.map(t => true))

    for (let i = 0; i < SET.goodTokens.length; i++) {
      const [cliffFace,approved,blocked] = await SET.morgothTokenApprover.baseTokenMapping(SET.goodTokens[i].address)
      expect(cliffFace).to.equal(ethers.constants.AddressZero)
      expect(approved).to.be.false
      expect(blocked).to.be.false
    }

    let choiceAfterFirst = await SET.configureTokenApproverPower.getChoice()
    expect(choiceAfterFirst.toString()).to.equal("3")
    ...
    await SET.configureTokenApproverPower.connect(SET.secondary).setApproveOrBlock(SET.badTokens.map(t => t.address), SET.goodTokens.map(t => false),SET.goodTokens.map(t => false))
    await SET.angband.connect(SET.secondary).executePower(SET.configureTokenApproverPower.address)


    for (let i = 0; i < SET.goodTokens.length; i++) {
      const isApproved = !(i === 2 || i === 4 || i === 5)
      const [,approved,] = await SET.morgothTokenApprover.baseTokenMapping(SET.goodTokens[i].address)
  ...
  it("t3. MorgothTokenApprover creates CliffFace and LimboProxy and ConfigureTokenApprover unmaps them", async function () {
    ...
    const lachesisLiteFactory = await ethers.getContractFactory("LachesisLite")
    const lachesisLite = await deploy<Types.LachesisLite>(lachesisLiteFactory)
    const liquidityReceiverFactory = await ethers.getContractFactory("LiquidityReceiver")
    const liquidityReceiver = await deploy<Types.LiquidityReceiver>(liquidityReceiverFactory, lachesisLite.address, bigConstants.address)

    const blockIndexes = [3, 4, 7]
    const tokensToBlock = [SET.goodTokens[3], SET.goodTokens[4], SET.goodTokens[7]]
      .map(t => t.address)

    await SET.configureTokenApproverPower.connect(SET.secondary).setApproveOrBlock(tokensToBlock,tokensToBlock.map(t=>true), tokensToBlock.map(t => true))
    await SET.angband.connect(SET.secondary).executePower(SET.configureTokenApproverPower.address)

    await SET.configureTokenApproverPower.connect(SET.secondary).setUpdateConfig(SET.tokenProxyRegistry.address, SET.goodTokens[0].address, SET.owner.address, SET.owner.address, SET.owner.address)
    await SET.angband.connect(SET.secondary).executePower(SET.configureTokenApproverPower.address)
    ...
    let [approverCliffFaceMapping0,,] = await SET.morgothTokenApprover.baseTokenMapping(SET.goodTokens[0].address)
    let [approverCliffFaceMapping1,,] = await SET.morgothTokenApprover.baseTokenMapping(SET.goodTokens[1].address)
   ...
   let [approverCliffFaceMapping0After,,] = await SET.morgothTokenApprover.baseTokenMapping(SET.goodTokens[0].address)
    let [approverCliffFaceMapping1After,,] = await SET.morgothTokenApprover.baseTokenMapping(SET.goodTokens[1].address)

    expect(approverCliffFaceMapping0After).to.equal(proxyConfigForToken0.behodlerProxy)
    expect(approverCliffFaceMapping1After).to.equal(proxyConfigForToken1.behodlerProxy)
