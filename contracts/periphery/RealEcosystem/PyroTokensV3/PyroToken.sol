...
contract PyroToken is PyroERC20, RG.ReentrancyGuard {
  ...
  function setObligationFor(
    ...
    debtObligations[borrower] = DebtObligation(baseTokenBorrowed, pyroTokenStaked, rate, block.timestamp);
    ...
    if (netStake > 0) {
      ...
      if (borrowerPyroBalance < stake) {
        revert Error.StakeFailedInsufficientBalance(borrowerPyroBalance, stake);
      }
      unchecked {
    ...
    if (netBorrowing > 0) {
      ...
      config.baseToken.safeTransferFrom(borrower, address(this), absoluteBorrowing);
    }
    emit LoanObligationSet(borrower, baseTokenBorrowed, pyroTokenStaked, rate, slashBasisPoints);
    success = true;
  }
  ...
  function _redeem(address recipient, address owner, uint256 amount) internal updateReserve returns (uint256) {
    uint256 _redeemRate = redeemRate();
