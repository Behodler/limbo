...
contract ProxyHandler {
  ...
  /**
     ...
     */
  function mintPyroFromBase(address pyroToken, uint256 baseAmount) public returns (uint256) {
    (, IERC20 proxyToken, , ) = PyroTokenLike(pyroToken).config();
    TokenProxyBaseLike proxy = TokenProxyBaseLike(address(proxyToken));
  ...
  /**
     ...
     */
  function redeemFromPyro(address pyroToken, uint256 pyroAmount) public returns (uint256) {
