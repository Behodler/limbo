...
contract Angband is Empowered, Thangorodrim {
  ...
  function resetEmergencyCooldownPeriod(uint256 daysFromNow) public requiresPower(powersRegistry.WIRE_ANGBAND()) {
  ...
  modifier ensureOwnershipReturned(address powerInvoker) {
    ...
    require(
      ownable == address(this) || Ownable_071(ownable).owner() == address(this),
      "MORGOTH: power invoker failed to return ownership"
    );
  }

  function authorizeInvoker(address invoker, bool authorized) public requiresPower(powersRegistry.AUTHORIZE_INVOKER()) {
    authorizedInvokers[invoker] = authorized;
  }

  function setPowersRegistry(address _powers) public requiresPower(powersRegistry.WIRE_ANGBAND()) {
    powersRegistry = PowersRegistry(_powers);
    _setAddress(POWERREGISTRY, _powers);
  }

  function mapDomain(address location, bytes32 domain) public requiresPower(powersRegistry.WIRE_ANGBAND()) {
    require(
  ...
  function relinquishDomain(bytes32 domain) public requiresPower(powersRegistry.WIRE_ANGBAND()) {
    address domainContract = getAddress(domain);
    Ownable_071(domainContract).transferOwnership(msg.sender);
  }

  function setBehodler(address behodler, address lachesis) public requiresPower(powersRegistry.POINT_TO_BEHODLER()) {
    address self = address(this);
    _setAddress(BEHODLER, behodler);
    _setAddress(LACHESIS, lachesis);
    require(Ownable_071(behodler).owner() == self, "MORGOTH: transfer Behodler ownership to Angband");
    require(Ownable_071(lachesis).owner() == self, "MORGOTH: transfer Lachesis_071 ownership to Angband");
  ...
  function withdrawSCX(uint256 amount) public requiresPower(powersRegistry.TREASURER()) {
