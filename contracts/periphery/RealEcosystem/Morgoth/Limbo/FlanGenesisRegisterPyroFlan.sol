...
contract FlanGenesisRegisterPyroFlan is PowerInvoker {
  ...
  function orchestrate() internal override returns (bool) {
    ...
    PyroTokenLike pyroflan = PyroTokenLike(params.liquidityReceiver.baseTokenMapping(address(params.flan)));
    params.flan.approve(address(pyroflan), type(uint256).max);
