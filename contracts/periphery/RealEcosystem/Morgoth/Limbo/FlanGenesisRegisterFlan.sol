...
contract FlanGenesisRegisterFlan is PowerInvoker {
  ...
  constructor(
    ...
    if (uniPair == address(0)) {
      ...
      params.uniswapPair = UniswapPairLike(uniswapFactory.getPair(behodler, flan));
    }
    ...
    if (sushiPair == address(0)) {
      ...
      params.sushiPair = UniswapPairLike(sushiFactory.getPair(behodler, flan));
    }
  ...
  function orchestrate() internal override returns (bool) {
    uint256 daiBalanceOnBehodler = params.dai.balanceOf(address(params.scarcity));
    ...
    params.flan.approve(address(params.scarcity), type(uint256).max);
    params.scarcity.addLiquidity(address(params.flan), daiBalanceOnBehodler);
    uint256 daiToSend = params.scarcity.withdrawLiquidityFindSCX(address(params.dai), 1000, 25 ether, 20);
