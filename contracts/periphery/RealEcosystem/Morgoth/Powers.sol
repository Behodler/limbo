...
abstract contract PowerInvoker {
  ...
  modifier revertOwnership() {
    ...
    if (ownableContract != address(angband)) Ownable_071(ownableContract).transferOwnership(address(angband));
  }
...
contract Empowered is Ownable_071 {
  ...
  modifier requiresPowerOnInvocation(address invoker) {
    ...
    require(initialized, "MORGOTH: powers not allocated.");
    bool userHas = powersRegistry.userHasPower(power, msg.sender);
    require(userHas, "MORGOTH: forbidden power");
    _;
  }
