...
import "../../../openzeppelin/Ownable.sol" as OZ;
...
abstract contract PowerInvoker {
  ...
  modifier revertOwnership() {
    ...
    if (ownableContract != address(angband)) OZ.Ownable(ownableContract).transferOwnership(address(angband));
  }
...
contract Empowered is OZ.Ownable {
  bool public constant REAL = true;
