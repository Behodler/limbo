...
contract UniswapV2Router02 {
  ...
  function _swap_new(uint256[] memory amounts, address[] memory path, address _to) internal virtual {
  ...
  //adaptation for sol 8.13 means create2 inference is different
  function pairFor(address input, address output) internal view returns (address) {
    return IUniswapV2Factory(factory).getPair(input, output);
  }
  ...
  function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
    for (uint256 i; i < path.length - 1; i++) {
      ...
      uint256 amountOutput;
      {
        ...
        amountInput = UNIIERC20(input).balanceOf(address(pair)).sub(reserveInput);
  ...
  function swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    uint256 balanceBefore = UNIIERC20(path[path.length - 1]).balanceOf(to);
    _swapSupportingFeeOnTransferTokens(path, to);
    require(
      UNIIERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
  ...
  function swapExactTokensForETHSupportingFeeOnTransferTokens(
    ...
    uint256 amountOut = UNIIERC20(WETH).balanceOf(address(this));
